...
def admin_noticias_guardar():
    ...
    datos = (
        _nombre, _apellido, _documento, _telefono, _descripcion, _nombreEs, _direccion, _clasificacion, _correoelec,
        _notificacion, _fecha, nuevoNombre)
    conexion = mysql.connect()
    cursor = conexion.cursor()
...
def usuarios_noticias_guardar():
    ...
    sqlfp = "Insert Into `form_princ` (`id_fp`, `nombre`, `apellido`, `documento`,`telefono`,`descripcion`, " \
            "`nom_esta`, `dir_esta`, `cla_esta`,`correo`,`not_esta`, `fecha`,`imagen`) Values (Null,%s,%s,%s,%s," \
            "%s,%s,%s,%s,%s,%s,%s,%s);"
    datos = (
        _nombre, _apellido, _documento, _telefono, _descripcion, _nombreEs, _direccion, _clasificacion, _correoelec,
        _notificacion, _fecha, nuevoNombre)
